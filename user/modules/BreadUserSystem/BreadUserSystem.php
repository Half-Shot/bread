		...
		foreach($this->settings->adminPanelSettings->usereditForm as $element)
		{
			if(isset($element->informationKey))
			{
				if($element->informationKey !== false){
					...
					if($element->informationKey === "groups"){
						//Add all groups
						$element->dataset = array();
						foreach(array_keys($this->groups) as $group)
						{
							$DataSetValue = new \Bread\Structures\BreadFormOption();
							$DataSetValue->label = $group;
							$DataSetValue->value = $group;
							$element->dataset[] = $DataSetValue;
						}
                                                $NoGroupOverride = new \Bread\Structures\BreadFormOption();
                                                $NoGroupOverride->label = "None (Will Override)";
                                                $NoGroupOverride->value = "%NOGROUP%";
                                                $element->dataset[] = $NoGroupOverride;
					}
				}
