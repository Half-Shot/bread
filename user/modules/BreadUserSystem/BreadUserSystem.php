...
class BreadUserSystem extends Module {
    ...
    function CheckSession() {
        ...
        if (!is_int($_SESSION["uid"]) || $_SESSION["REMOTE_ADDR"] != $_SERVER["REMOTE_ADDR"] || $_SESSION["HTTP_USER_AGENT"] != $_SERVER["HTTP_USER_AGENT"]) {
    ...
    function ConstructUserPanel() {
        ...
        foreach ($this->settings->adminPanelSettings->usereditForm as $element) {
            if (isset($element->informationKey)) {
                ...
                if ($element->informationKey == "groups" && $element->type == "dropdown") {
                    $element->dataset = array_keys($this->groups);
                }
    ...
    function AjaxUserInfo() {
        ...
        foreach ($returnData as $uid => $val) {
            ...
            foreach ($this->settings->adminPanelSettings->usereditForm as $formElement) {
                ...
                if ($key == "groups") {
                    $returnData[$uid][$key] = $CurrentUser->groups;
                } else if (isset($CurrentUser->information->$key)) {
    ...
    function ValidatePassword($args) {
        ...
        //Don't allow more than 256 cause that's CRAZY
        if(count($args["data"]) > 256){
            return false;
        }
	...
	function ChangeGroups($args){
		...
		if(!$this->CheckUserEditPerms($user,"groups"))
			return false;
                ...
                if(is_string($args["value"]))
		{
                ...
                else if(!is_array($args["value"])){
                    return false;
                }
                if(in_array("%NOGROUP%", $args["value"])){
                    $user->groups = array();
                }
                else{
                    $user->groups = $args["value"];
                }
	...
	function ValidateGroups($args){
		if(is_string($args["data"]))
		{
                    $args["data"] = array($args["data"]);
		}
                else if(!is_array($args["data"])){
                    return false;
                }
		foreach($args["data"] as $group){
			if(!array_key_exists($group, $this->groups) && $group !== "%NOGROUP%"){
				return false;
			}
		}
                if($args["data"] == $args["user"]->groups){
                    return false;
                }
	...
	function AjaxWriteUserInfo(){
		...
		$data = Util::Jquery_DeSerializeFormArray($_POST["data"]);
                $ReportingObject = new \stdClass();
		foreach ($users as $uid){
                    $ReportingObject->$uid = new \stdClass(); 
                    $ReportingObject->$uid->validation = new \stdClass();
                    $ReportingObject->$uid->changed = new \stdClass();
                    $user = $this->GetUserByUID($uid);
                    if($user == false)
                    ...
                    foreach($this->settings->adminPanelSettings->usereditForm as $formElement){
                        ...
                        else
                        {
                                ...
                                if(isset($user->information->$key)){
                                    if($user->information->$key === $value){
                                            continue; //Nothing to do here.
                                            $ReportingObject->$uid->changed->$key = false;
                                    }
                                }
                        }
                        $ReportingObject->$uid->validation->$key = $validatedOk;
                        if(!$validatedOk){
                                continue;
                        }
                        ...
                        {
                                $couldChange = $this->manager->FireEvent("Bread.Security.ChangeGroups",array("user"=>$user,"uid"=>$uid,"value"=>$value));
                        }
                        else{
                        ...
                        $ReportingObject->$uid->changed->$key = $couldChange;
                    }
		}
		Site::$settingsManager->SaveSetting($this->userDB,"breadusersystem#users");
                $ReturnedBody = json_encode($ReportingObject);
                Site::AddToBodyCode($ReturnedBody);
		return $ReturnedBody;
	}
...
class BreadUserSystemAdminPanelSettings {
    ...
    function __construct() {
        ...
        $Groups->type = "dropdown";
        $Groups->multiple = true;
        $Groups->informationKey = "groups";
        $Groups->multiuser = true;
        $Groups->validationEvent = "BreadUserSystem.ValidateGroups";
        $Groups->validationModule = "BreadUserSystem";
